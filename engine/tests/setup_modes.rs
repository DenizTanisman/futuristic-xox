...
fn classic_hands_fill_the_board() {
    let (_m, s) = build(GameConfig { kind: ModeKind::Classic, rows: 3, cols: 3 }, 0);
    ...
    let (_m, s4) = build(GameConfig { kind: ModeKind::Classic, rows: 4, cols: 4 }, 0);
...
fn original_hands_match_spec_counts() {
    let (_m, s3) = build(GameConfig { kind: ModeKind::Original, rows: 3, cols: 3 }, 0);
    assert_eq!(s3.hands[0], (1..=6).collect::<Vec<u8>>());
    assert_eq!(s3.hands[1], (1..=6).collect::<Vec<u8>>());

    let (_m, s4) = build(GameConfig { kind: ModeKind::Original, rows: 4, cols: 4 }, 0);
...
fn morph_hands_have_two_of_each_value() {
    let (_m, s4) = build(GameConfig { kind: ModeKind::Morph, rows: 4, cols: 4 }, 0);
    // 4×4 → values 1..6, two each = 12 pawns; two moves per turn.
    assert_eq!(s4.hands[0].len(), 12);
    assert_eq!(s4.moves_left_in_turn, 2);
    for v in 1..=6u8 {
        assert_eq!(s4.hands[0].iter().filter(|&&x| x == v).count(), 2);
    }

    let (_m, s5) = build(GameConfig { kind: ModeKind::Morph, rows: 5, cols: 5 }, 0);
    assert_eq!(s5.hands[0].len(), 22); // values 1..11, two each
}

#[test]
fn bonanza_is_deterministic_per_seed() {
    let a = build(GameConfig { kind: ModeKind::Bonanza, rows: 3, cols: 3 }, 12345).1;
    let b = build(GameConfig { kind: ModeKind::Bonanza, rows: 3, cols: 3 }, 12345).1;
...
fn bonanza_preserves_the_pawn_pool() {
    ...
    for seed in 0..50u64 {
        let s = build(GameConfig { kind: ModeKind::Bonanza, rows: 3, cols: 3 }, seed).1;
...
fn bonanza_k_zero_case_can_occur() {
    ...
    for seed in 0..200u64 {
        let s = build(GameConfig { kind: ModeKind::Bonanza, rows: 4, cols: 4 }, seed).1;
